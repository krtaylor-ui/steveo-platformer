...
module.exports = function setupWorldsRoutes(app) {
  ...
  // §Custom Sprites — a world's chosen character (world_data.characterId) is persisted through the
  // existing full-world PUT above (the client GETs the world, merges characterId, and PUTs it back),
  // so no dedicated route is needed — it works even on an API server that predates this feature.
