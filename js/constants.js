...
const GAME_VERSION = 'v3 · build 245 (§Moving Platforms §1 — THE RAIL: a placeable waypoint path (reuses the Travel-Tube click-corners UX + geometry) that platforms will ride. Rail palette block in the Classic/Mechanics tab; click corners, click the last point again to finish (or the FIRST to close a LOOP). Click a placed rail → modal to cycle Visibility (Visible+Solid / Visible+Non-solid (default) / Invisible), toggle Loop, Edit Nodes (drag waypoints, double-click an end to extend, Delete to remove), or Delete. Solid rails paint standable RAIL grid cells. Pure math in moving-platform.js (advance/weight/centre-of-mass/ballistic/tilt/flood-fill) + 24 headless tests. Serialized across ALL load/save paths incl. the SpeedRunner gap + localStorage. PREV build 244 (redstone adjacency power)'; — a powered generator (lever, pressure plate, target, receiver, AND/NOR gate output, pulse converter) now drives a sink (lamp, TX, piston, trapdoor, TNT, target, gate input, converter) sitting RIGHT NEXT TO IT, with no redstone dust in between. Simple generators power all four neighbours; gates & converters power only their output side. Fully additive to the existing dust wiring. PREV build 243 (TX/RX overlays that co-exist with dust & gates). PREV build 242 (Pulse Converter placement handler — mount them on a solid block surface (click a block, like dust/gates) and they can now SHARE a cell with redstone dust or an AND/NOR gate. A TX co-located with dust transmits that dust directly; a RX powers dust on its own cell. Placed/removed/rendered/saved via the _transmitters/_receivers Maps; legacy worlds auto-migrate (old grid TX/RX cells convert to overlays on load). PREV build 242 (Pulse Converter placement handler — a freshly-placed converter creates its component immediately, so you can rotate it (click while Converter selected) right away instead of having to bounce through test mode first. That missing component was also why Toggle→Pulse failed: with no editor component the orientation defaulted to right in test mode, putting the lever on the wrong side; now that you can orient it, Toggle-side input emits a Pulse out the Pulse side as intended. Sandbox HOTBAR now survives a test-mode round-trip (GAME_STATE.serialize carries sbHotbar/sbHotbarSel, matching the saves.js path), so returning to the editor no longer empties it. Redstone LAMP off-state is now much darker (alpha 0.28) for stronger on/off contrast.)';
const CANVAS_W    = 800;
const CANVAS_H    = 500;
const BLOCK_SIZE  = 32;
