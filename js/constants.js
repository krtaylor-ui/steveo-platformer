...
const GAME_VERSION = 'v3 build 257 (Moving Platforms ride/collision/render pass, browser-untested): platform-mounted LAMPS/LEVERS now render their real on/off state (so an RX-powered lamp panel on a platform lights up). RIDE physics overhauled — riders persistently ATTACH to the platform and glue to its smooth surface, so verticals AND diagonals are smooth and you no longer fall through on a diagonal; an edge-grabber now follows the platform (both axes) instead of hanging in mid-air. Platform-vs-platform bounce fixed — on contact they reverse, shove apart by their combined spans, and cooldown, so they no longer snap together and vibrate. Destroy-Smaller now ANIMATES: the loser shatters into blocks that fling up, tumble, bounce off the ground, settle, and fade a few seconds later. Prev 256 lamp-panel conduction + TX/RX hide-in-play; 255 loop/angled/ride/collision/reposition.';
const CANVAS_W    = 800;
const CANVAS_H    = 500;
const BLOCK_SIZE  = 32;
