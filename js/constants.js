...
const GAME_VERSION = 'v3 build 383: CRITICAL fix: 3-player arena freeze — a secondary player (P2-P4) stepping onto a ladder ran player.js\'s UNGUARDED input.isDown(\'KeyW\'), but their per-player input adapter had no isDown, so it threw \'isDown is not a function\' and killed the game loop (the ~1-minute freeze = whenever someone touched a ladder). Adapter now provides isDown (maps up-keys → that player\'s up), isStickUp, isLookUpHeld — everything player.js reads off input';
