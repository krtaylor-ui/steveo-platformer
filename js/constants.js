...
const GAME_VERSION = 'v3 build 268 (MOVING-REDSTONE — the last piece): platform redstone now works stationary AND while moving, in either direction. The remaining bug: the dust CHAIN enqueued each step at a hardcoded +6 frames (not the step rate), so a moving platform crossed a cell mid-chain and the pending entries went stale → the dust under the lever powered but never chained to the next dust/lamp (the exact erratic, direction-dependent behavior). Now every propagation step honors _rsStepFrames(), and platforms with redstone force INSTANT propagation (0-frame steps, drained same-frame) so the whole circuit resolves each frame before the platform moves. Combined with 267 (getAt reindex + block-cell coords + collision-safe re-key), redstone rides platforms correctly. Normal (non-platform) redstone timing is unchanged.';
const CANVAS_W    = 800;
const CANVAS_H    = 500;
const BLOCK_SIZE  = 32;
