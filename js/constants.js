...
const GAME_VERSION = 'v3 build 262 (MOVING REDSTONE on platforms, browser-untested): redstone now RIDES a moving platform and works at its live position — levers, dust, gates, targets, converters, lamps, and TX/RX are captured when the platform lifts and repositioned each time it crosses a cell, so dust powers, panels light, and clicks land on the moving block. PRESSURE PLATES on a platform now actually press (excluded from the platform surface so you stand IN the plate cell, on the block below) and no longer look like you hover. Carried dust survives (no longer reaped as floating-on-air). The DIRECTION CONTROLLER now also accepts PHYSICAL left/right inputs (a powered dust/lever directly beside it), not just transmitter channels — so you can wire it on the platform. Carried redstone is cleaned up when a platform shatters. Prev 261 Launch-Ramp direction + power settings + inelastic debris.';
const CANVAS_W    = 800;
const CANVAS_H    = 500;
const BLOCK_SIZE  = 32;
