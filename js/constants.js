...
const GAME_VERSION = 'v3 build 382: Fix: P2-P4 controller players could not grab monkey-bars — the bar-grab read P1-only keyboard keys + isStickUp(), but secondary players get a limited per-player input adapter with none of those, so upHeld was always false for them. Added InputManager.pUp(i) (per-player left-stick/d-pad up), exposed isUp on the adapter, and made the bar accept UP or JUMP for every player';
