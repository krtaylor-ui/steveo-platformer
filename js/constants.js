...
const GAME_VERSION = 'v3 build 377: debug HUD shows a LIVE mob count on the perf line (the estimate line\'s \'mobs\' is a static construction-time snapshot that misled testing); add supported OverheadGame._spawnMob(type,col,row,opts) — builds a mob with the constructor\'s exact shape and adds it to the live this.mobs, opts.stationary = speed0+detect0 so it holds its cell (for the §42 occlusion visual, and the reusable spawn primitive for overhead Survival Waves)';
