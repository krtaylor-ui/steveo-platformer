...
const GAME_VERSION = 'v3 build 380: Fix: Travel/glass tubes (and all classic-block metadata) invisible-but-solid in arena worlds — buildArenaWorldDataFromSave dropped travelTubes/pipeLinks/blockContents/rails, and _buildLevel never ran _restoreClassicBlockData for the arena path, so the TUBE_WALL cells restored (blocking) while the visible tube band (drawn from travelTubes) did not. Now carried through + restored for arena (also fixes pipes/question-blocks/rails/moving-platforms in arenas)';
