...
const GAME_VERSION = 'v3 · build 255 (Moving Platforms feel/edit pass (browser-untested): LOOP is now automatic (connect the ends = loop; the broken manual toggle is gone). ANGLED rails AND tubes — a Path toggle in each modal connects waypoints as direct diagonals instead of forcing right angles. Smoother ride: riders now glue to the platform`s SMOOTH surface (no more cell-by-cell jerk up/down). Grabbing the back EDGE now counts as a rider (Rider-Powered/One-Touch start WITH you instead of leaving you behind); a GRAPPLE link never counts as a rider. Platform-vs-platform collision now triggers on ANY part touching (full-platform overlap, not just anchors). Reposition moves the WHOLE platform as one unit. Prev 254 — modal fix + live draft dots.'; every new config modal (Rail, Anchor, Pause Node, Direction Controller, Speed Segment, Rail Gate) was calling this._roundRect — which does not exist (it is a global _roundRect) — so each threw right after drawing the grey backdrop = "screen greys out, no options". All 14 calls fixed → the modals now render. Also the rail DRAFT dots now appear LIVE as you click (they were gated behind a rails-exist check so the first rail showed nothing until finished); dots now have a dark ring + white outline + a rubber-band line to the cursor + a start-point ring for closing a loop, so they never blend into the background. Prev 253 — RAIL GATE: paint Rail Gate cells on a rail; a CLOSED gate blocks a platform (it stops / reverses at the edge). Opens on: a Redstone channel (7 Gate mode) OR platform Weight ≥/≤ a threshold (10 Weight-Sensitive Trigger) — both reuse the shared weight system. DEFERRED (documented in roadmap): the Switch/fork BRANCHING mode and the 4 animated visual styles (drawbridge/rise/extend/dissolve + the arrived-too-early→fall race) — the rail model is linear this session. Prev 252 — CENTER OF GRAVITY tilt: opt-in per-platform Advanced toggle in the Anchor modal. The platform tilts from the horizontal offset between the anchor and the weighted centre of mass (shared 6/9/13 weight system); a rider standing on it adds a point mass, so you may need to stand in the right spot to balance. The tilt rotates BOTH the render and the (cell-rounded) collision cells around the anchor. DECISION: riders DO slide downhill on a steep tilt (a gentle nudge past ~9°) — flagged, not silent. Works as a static seesaw on a 1-point rail. Prev 251 — LAUNCH PLATFORM ballistic sim: place a Launch Ramp on a rail to mark a launch point. A platform reaching it is flung as a real rigid-body projectile — exit velocity = its attained speed (incl. Speed Segments) along the travel heading, tipped upward by the ramp, then gravity acts on the whole block group. Mid-flight it catches the FIRST rail its anchor touches (resumes riding); smashing into solid terrain, or sailing out of bounds, shatters it (blocks scatter like the death effect and fall off-screen). Riders ride the flying platform. No designer pairing — outcome emerges from speed + geometry. Prev 250 — SPEED CONTROL SEGMENT: paint Speed Segment cells on a rail (adjacent cells auto-merge into one zone); click to set the zone Target Speed. A platform passing through smoothly ramps (smoothstep-eased) from its current speed toward the target across the zone length, then PERSISTS that speed — a genuine value on the platform, so an upcoming Launch Ramp reads whatever speed it actually attained. Prev 249 — DIRECTION CONTROLLER: a block placed on a platform with two inputs, Left and Right, each fed by a redstone TRANSMITTER channel (wire any source → transmitter → channel; wireless because a moving platform cannot stay wired to static grid dust). Left → travel Backward, Right → Forward, BOTH together → TOGGLE. The equality check is EDGE-triggered (evaluated only when an input actually changes) so an idle both-LOW never counts as equal and toggles every frame — deliberate per Q1. Same channel on both inputs = a one-source toggle. Prev 248 — MULTIPLE PLATFORMS + WEIGHT COLLISION: one rail hosts many platforms (each its own Anchor). Rail modal adds "Platforms Meet": Pass Through / Redirect (both reverse) / Destroy Smaller (heavier crushes lighter; TIE = Redirect). Weight is a per-block-type weighted sum (default 1 each = block count today) built once and shared by 6/9/13. Prev 247 — PAUSE NODES: while Editing a rail`s Nodes, right-click a waypoint to set its pause: No Pause / Duration (1/2/3/5/8s) / Pause-until-Anchor-reactivated. A platform crossing that waypoint parks on it and waits — the duration timer, or a fresh rising edge on its anchor signal (use Signal Response = Toggle so there is a discrete re-pulse; warned in-modal). Pause nodes show as blue diamonds in the editor. Prev 246 — ANCHOR + PLATFORM + CARRYING. Place an Anchor Block ON a rail to bind a PLATFORM = the connected block group flood-filled from the anchor. Click the anchor for a config modal: Initial Direction, Movement Mode (Continuous / Rider-Powered / One-Touch / Redstone-Controlled + Sustained|Toggle signal response via a built-in hidden receiver), Return Mode (One-Way / Round-Trip), Speed, Reposition, Remove. In PLAY the group lifts out of the static grid and rides the rail; its cells register as solid (rounded) so all existing physics treat it as ground, and riders (players/mobs/items) standing on it are carried by the per-frame delta + depenetration. Build platforms DETACHED from terrain (a runaway flood-fill >300 blocks is refused). Prev 245: the Rail. Prev 242-244: redstone fixes, TX/RX overlays, adjacency power.)';
const CANVAS_W    = 800;
const CANVAS_H    = 500;
const BLOCK_SIZE  = 32;
