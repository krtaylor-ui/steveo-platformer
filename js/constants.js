...
const GAME_VERSION = 'v3 build 376: debug HUD flood fix — the HUD printed GAME_VERSION verbatim, but bump-build had grown that string to ~80k chars (unbounded prepend), burying fps/frame/worst/cells off-screen. GAME_VERSION is now the current build note only (full changelog lives in git log); the HUD extracts just \'v3 build NNN\'';
