...
const WORLD_SETTINGS = {
  ...
  get SETTINGS() {
    ...
    return [
      ...
      { key: 'barTraverseStyle', tab: 'movement', group: 'Moves', modes: M.physics, type: 'cycle', opts: ['smooth', 'brachiation', 'bigswing', 'lunge', 'compactswing', 'compactlunge'], dflt: 'compactswing', label: 'Bar: Traverse Style', fmt: (v) => ({ smooth: 'Smooth', brachiation: 'Brachiation', bigswing: 'Big Swing', lunge: 'Lunge', compactswing: 'Compact Swing', compactlunge: 'Compact Lunge' }[v] || v), sub: true, advanced: true, hint: 'the hand-over-hand animation. Compact Swing (default) = Big Swing’s body/leg motion with tight handholds; Compact Lunge = Lunge’s leg kick with tight handholds; plus Smooth · Brachiation · Big Swing · Lunge' },
  ...
  _weaponRows(M, O, x1) {
    ...
    rows.push({ key: 'weaponGrapple', tab: 'movement', group: gh, modes, type: 'toggle', dflt: false, label: 'Configure Grappling Hook', hint: 'the Grappling Hook is always available under Equipment in the Sandbox palette. The player must PICK ONE UP to use it — fire a cable, swing, reel in, climb 1-block ledges. Look-Up Aim (Up/W = aim up, jump → J) turns on once you are holding it. Turn this ON to customize the range.' });
    rows.push({ key: 'grappleRange', tab: 'movement', group: gh, modes, type: 'cycle', opts: [6, 8, 10, 12, 16], dflt: 8, label: 'Range', fmt: (v) => v + ' bl', sub: true, dependsOn: 'weaponGrapple', advanced: true, hint: 'hook reach; nothing hit within range → it auto-retracts' });
    rows.push({ key: 'grappleDamage', tab: 'movement', group: gh, modes, type: 'cycle', opts: [0, 2, 4, 6, 9], dflt: 0, label: 'Hook Damage', fmt: (v) => v === 0 ? 'None (knockback only)' : v, sub: true, dependsOn: 'weaponGrapple', advanced: true, hint: 'damage dealt when the hook hits an enemy (it always knocks back + returns); 0 = no damage' });
    ...
    rows.push({ key: 'grappleSwingAssist', tab: 'movement', group: gh, modes, type: 'cycle', opts: ['none', 'lean', 'pump'], dflt: 'lean', label: 'Swing Assist', fmt: (v) => ({ none: 'None (raw physics)', lean: 'Lean (hold ← →)', pump: 'Pump (time ← →)' }[v] || v), sub: true, dependsOn: 'weaponGrapple', hint: 'accelerate a swing with Left/Right: Lean = steady push while held; Pump = a timing boost when you press with the swing at the bottom of the arc; None = pure pendulum' });
    rows.push({ key: 'grappleSwingStrength', tab: 'movement', group: gh, modes, type: 'cycle', opts: [0.25, 0.5, 1, 1.5, 2], dflt: 0.5, label: 'Swing Assist Strength', fmt: (v) => v.toFixed(2).replace(/\.?0+$/, '') + 'x', sub: true, dependsOn: 'weaponGrapple', advanced: true, hint: 'how hard Lean/Pump push the swing (default 0.5 — the old feel was 1x and looped the sprite around). Gravity + a hard arc limit stop you swinging over the top, so higher just builds momentum faster' });
    return rows;
  },
