...
class Game {
  ...
  _updateDragon() {
    ...
    const BOB_OFFSETS = [-1, -0.5, 0, 0.5, 1];
    ...
    while (d._distTracker >= BLOCK_SIZE) {
      ...
      d.y += BOB_OFFSETS[d.animationFrame];
    }
  ...
  _renderDragon(ctx) {
    ...
    const playerIsToRight = (this.player.x > d.x);
    ...
    if (playerIsToRight) {
      ...
      headWorldX = (d.direction === 'left')
        ? d.x + DRAGON_BODY_W + 20   // looking backward: apply blank-space offset
        : d.x + DRAGON_BODY_W;       // looking forward: no offset
    } else {
      // Head faces LEFT
      headRotation = DRAGON_HEAD_ROT;
      headWorldX = (d.direction === 'right')
        ? d.x - DRAGON_HEAD_W - 20   // looking backward: apply blank-space offset
        : d.x - DRAGON_HEAD_W;       // looking forward: no offset
    }

    const headWorldY  = d.y + 12;
    const pivotSx     = Math.floor(headWorldX - this.camera.x);
    const pivotSy     = Math.floor(headWorldY - this.camera.y + 20); // pivot 20px down from sprite top
