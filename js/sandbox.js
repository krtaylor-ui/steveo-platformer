...
const SANDBOX_PALETTE_BLOCKS = {
  ...
  mechanics: [
    BLOCK.LADDER, BLOCK.ONEWAY_PLATFORM, BLOCK.TRAMPOLINE, BLOCK.ICE, BLOCK.GLASS,
    ...
    BLOCK.BAR, BLOCK.BAR_PLATFORM, BLOCK.TUBE_WALL,
    BLOCK.RAIL, BLOCK.ANCHOR_BLOCK, BLOCK.DIRECTION_CONTROLLER,
    BLOCK.SPEED_SEGMENT, BLOCK.LAUNCH_RAMP, BLOCK.RAIL_GATE, BLOCK.RAIL_SWITCH,
  ],
};
  ...
  _drawPlacedItems(ctx, camera, frameCount) {
    for (let i = 0; i < this.placedItems.length; i++) {
      ...
      if (it.blockType) {
        ...
        ctx.textAlign    = 'center';
        ctx.textBaseline = 'middle';
        if (!(data?.piece ? drawArmorIcon(ctx, data.piece, cx, cy, 22, data.color)
                          : (data?.weaponClass && drawWeaponIcon(ctx, data.weaponClass, cx, cy, 22, data.color)))) ctx.fillText(sym, cx, cy);
      }
      ctx.restore();
    }
  }
    ...
    for (let i = 0; i < SB_SLOTS; i++) {
      ...
      if (entry !== null) {
        if (entry.kind === 'block') {
          ...
          ctx.textAlign    = 'center';
          ctx.textBaseline = 'middle';
          if (!(data?.piece ? drawArmorIcon(ctx, data.piece, ecx, ecy, 22, data.color)
                            : (data?.weaponClass && drawWeaponIcon(ctx, data.weaponClass, ecx, ecy, 22, data.color)))) ctx.fillText(sym, ecx, ecy);
          ctx.textAlign    = 'left';
          ctx.textBaseline = 'alphabetic';
        } else if (entry.kind === 'blockItem') {
  ...
  _drawPalette(ctx, input) {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (isSpecialTab && itm.kind === 'tool') {
        ...
        if (!(itm.piece ? drawArmorIcon(ctx, itm.piece, cxc, cyc + 9, 22, itm.color)
                        : (itm.weaponClass && drawWeaponIcon(ctx, itm.weaponClass, cxc, cyc + 9, 22, itm.color)))) ctx.fillText(sym, cxc, cyc + 9);
        if (hov) {
          ctx.fillStyle    = itm.color;
          ctx.font         = '7px Courier New';
          ctx.textBaseline = 'bottom';
