// ============================================================
// server-multiplayer.js — Node.js + Socket.io multiplayer server
...
const games   = new Map();  // gameId → GameEntry (discovery layer)
...
// In-memory log of recently deleted games (last 200)
...
// Default outfit colours matching PLAYER_COLORS on client
...
// ── Server-side save migration ────────────────────────────────
// Normalises v1 server saves (multiplierSettings, no saveVersion)
// to v2 (worldAdvSettings, saveVersion:2).  Keeps sparse blocks.
...
function batchConvertOldSaves() {
  if (!fs.existsSync(SAVES_DIR)) {
    console.log('[Migration] No saves directory — nothing to convert');
    return;
  }
  ...
  if (files.length === 0) {
    console.log('[Migration] No save files found');
    return;
  }
  ...
  for (const file of files) {
    ...
    try {
      ...
      if (data.saveVersion === 2) { skipped++; continue; }

      // Back up original before overwriting
      fs.writeFileSync(filePath.replace('.json', '.bak.json'), raw);

      const migrated = migrateSaveData(data);
      fs.writeFileSync(filePath, JSON.stringify(migrated, null, 2));
      converted++;
      console.log(`[Migration] Converted: ${file}`);
    } catch (e) {
      failed++;
      console.error(`[Migration] Failed to convert ${file}: ${e.message}`);
    }
  }

  console.log(`[Migration] Complete — ${converted} converted, ${skipped} already v2, ${failed} failed`);
}

// ── Grid → sparse blocks conversion ──────────────────────────
// Local saves store world as a full 2D grid[row][col]; the server
// uses a sparse "col,row" → blockType map.  Convert on receipt.
...
io.on('connection', socket => {
  ...
  socket.on('joinWorld', data => {
    ...
    const isCreator = !!(ge && ge.creatorBrowserId && ge.creatorBrowserId === browserId);
...
// Create a new browsable game session
app.post('/api/createGame', (req, res) => {
  const { creatorName, creatorBrowserId, mode = 'sandbox', worldName = 'New World',
          maxPlayers = 4, password } = req.body || {};

  const gameId = `g-${Date.now()}-${Math.random().toString(36).slice(2,5)}`;

  games.set(gameId, {
    gameId,
    worldId:          gameId,
    creatorName:      creatorName || 'Player',
    creatorBrowserId: creatorBrowserId || null,
    mode:             mode.toLowerCase(),
    worldName:        worldName || 'New World',
    maxPlayers:       Math.min(4, Math.max(2, parseInt(maxPlayers) || 4)),
    currentPlayers:   0,
    hasPassword:      !!password,
    passwordHash:     password ? hashPassword(password) : null,
    createdAt:        Date.now(),
    lastAccessedAt:   Date.now(),
    playerStates:     {},   // browserId → { inventory, hp, x, y } for Normal mode
  });

  console.log(`  Game created: ${gameId} (${mode}, max ${maxPlayers})`);
  res.json({ success: true, gameId });
});

// List available games (filterable by mode)
app.get('/api/listGames', (req, res) => {
  const modeFilter = (req.query.mode || 'all').toLowerCase();
  const now = Date.now();
  const msPerDay = 86400000;

  let list = [...games.values()].filter(g =>
    modeFilter === 'all' || g.mode === modeFilter
  );
  list.sort((a, b) => b.createdAt - a.createdAt);

  res.json(list.map(g => {
    const lc = LIFECYCLE[g.mode] || LIFECYCLE.sandbox;
    const daysSince = (now - (g.lastAccessedAt || g.createdAt)) / msPerDay;
    const daysRemaining = Math.max(0, lc.displayDays - daysSince);
    return {
      gameId:         g.gameId,
      ...
      lastAccessedAt: g.lastAccessedAt || g.createdAt,
      daysRemaining:  parseFloat(daysRemaining.toFixed(1)),
      isFull:         g.currentPlayers >= g.maxPlayers,
    };
  }));
});

// Validate password before client attempts socket join
app.post('/api/joinGame', (req, res) => {
  ...
  if (game.hasPassword && !verifyPassword(password || '', game.passwordHash))
    return res.json({ success: false, error: 'Incorrect password' });

  // Return mode so client knows which game type to launch
  const worldState = worlds.get(gameId)?.toJSON() || null;
  res.json({ success: true, mode: game.mode, worldName: game.worldName, worldState });
});

// Download world JSON
app.post('/api/downloadWorld', (req, res) => {
  const { worldId } = req.body || {};
  const world = worlds.get(worldId);
  if (world) return res.json(world.toJSON());
  const saved = loadWorldFromDisk(worldId);
  if (saved) return res.json(saved);
  res.status(404).json({ error: 'World not found' });
});

// Active worlds (debug / admin)
app.get('/api/worlds', (req, res) => {
  res.json([...worlds.values()].map(w => ({
    worldId: w.worldId, name: w.name, playerCount: w.getPlayerCount(),
    players: [...w.players.values()].map(p => ({ name: p.name, number: p.number })),
  })));
});

// Delete a game (creator or admin)
app.post('/api/deleteGame', (req, res) => {
  const { gameId, browserId, adminCode } = req.body || {};
  const ge = games.get(gameId);
  if (!ge) return res.json({ success: false, error: 'Game not found' });

  const isAdmin   = adminCode === 'DOGS';
  const isCreator = ge.creatorBrowserId && ge.creatorBrowserId === browserId;
  ...
  if (world) {
    io.to(gameId).emit('gameDeleted', { reason: 'Game was deleted by the owner' });
    worlds.delete(gameId);
  }

  logDeletion({
    gameId,
    mode:        ge.mode,
    worldName:   ge.worldName,
    creatorName: ge.creatorName,
    deletedBy:   isAdmin ? 'admin' : 'creator',
  });

  games.delete(gameId);
  console.log(`  Game ${gameId} deleted by ${isAdmin ? 'admin' : 'creator'}`);
  res.json({ success: true });
});

// Admin: list all games + deletion log
app.get('/api/admin/games', (req, res) => {
  if (req.query.adminCode !== 'DOGS')
    return res.status(403).json({ error: 'Not authorised' });

  const now = Date.now(), msPerDay = 86400000;
  res.json({
    active: [...games.values()].map(g => {
      const lc = LIFECYCLE[g.mode] || LIFECYCLE.sandbox;
      return {
        gameId:         g.gameId,
        mode:           g.mode,
        ...
        createdAt:      g.createdAt,
        lastAccessedAt: g.lastAccessedAt || g.createdAt,
        daysSince:      parseFloat(((now - (g.lastAccessedAt || g.createdAt)) / msPerDay).toFixed(1)),
        deletesInDays:  parseFloat(Math.max(0, lc.displayDays - (now - (g.lastAccessedAt || g.createdAt)) / msPerDay).toFixed(1)),
      };
    }),
    deleted: deletedGamesLog.slice(0, 50),
  });
});
...
function getTimeUntilMidnightEST() {
  ...
  return midnight.getTime() - est.getTime();
}

function runCleanupJob() {
  const now = Date.now(), msPerDay = 86400000;
  let deleted = 0;
  for (const [id, ge] of games) {
    const lc        = LIFECYCLE[ge.mode] || LIFECYCLE.sandbox;
    const daysSince = (now - (ge.lastAccessedAt || ge.createdAt)) / msPerDay;
    if (daysSince >= lc.deleteDays) {
      logDeletion({ gameId: id, mode: ge.mode, worldName: ge.worldName,
                    creatorName: ge.creatorName, deletedBy: 'cleanup',
                    daysSince: parseFloat(daysSince.toFixed(1)) });
      const world = worlds.get(id);
      if (world) { saveWorldToDisk(id, world); worlds.delete(id); }
      games.delete(id);
      deleted++;
    }
  }
  console.log(`[Cleanup] ${new Date().toISOString()} — deleted ${deleted} expired game(s)`);
}

function startCleanupJob() {
  const ms = getTimeUntilMidnightEST();
  console.log(`[Cleanup] First run in ${Math.round(ms / 60000)} min`);
  setTimeout(() => {
    runCleanupJob();
...
// ── Start ─────────────────────────────────────────────────────
server.listen(PORT, () => {
  console.log(`Steveo Platformer server running on port ${PORT}`);
  console.log(`Open http://localhost:${PORT} to play`);
  batchConvertOldSaves();
  startCleanupJob();
});
